...
class Hopital(Equipement):
    def __init__(self, nom = "hopital", prod = -300, effa = 10., activite = 50.):
        ...
        self.effacement = 0.
        self.temps_effa = 0 # temps pendant lequel l'hÃ´pital s'est effacÃ©
        self.temps_dernier_effa = 0 # temps Ã©coulÃ© depuis le dernier effacement
        self.cout = 10. # coÃ»t arbitraire, de l'ordre de celui de 100 maisons : on efface l'hÃ´pital le moins souvent possible
        

    def simulation(self):
        prod_min = self.production[Global.temps + 1] # pas d'effacement 
        if (self.temps_effa >= 3 or self.temps_dernier_effa <18):  # on ne peut pas effacer l'hÃ´pital pendant plus de 30 minutes et il faut attendre 3h avant de l'effacer de nouveau
            prod_max = prod_min                                    #j'aime l'idÃ©e
            cout_max = 0
        else:
