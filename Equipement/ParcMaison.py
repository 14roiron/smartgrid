...
class ParcMaison (Utilitaire) : 
    def __init__(self, nom, prod=-2.0, effa=0.1, activite=0., nb=300.): #consommation moyenne de environ 1kW/maison -->heure basse 0,7kW/maison
        ...
        self.cout=self.effacement/100.0*self.EFFA_MAX*(80.0/1000/6)*self.nombre
        self.nom=nom
        self.production=[]
        for i in range(0,721):
            self.production.append(50*(1+cos(pi/144.0*(i+30.0))*cos(3.0*(pi/144.0*(i+30))))) #pourcentage qui multiplié par self.PROD_MAX (<0) donne la production (<0)
        for i in range(721,1008):
            self.production.append(50*(1+cos(pi/72.0*(i-792))))
    ...
    def prevision(self, consigne=0, effacement=0):
        ...
        if p>=-effacement*self.EFFA_MAX/self.PROD_MAX:
            return (p+effacement*self.EFFA_MAX/self.PROD_MAX,effacement/100.0*self.EFFA_MAX*(80.0/1000/6)*self.nombre)
        else :
            return (0,-p/100.0*self.PROD_MAX*(80.0/1000/6)*self.nombre)
