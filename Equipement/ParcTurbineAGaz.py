    ...
    def prevision(self,consigne=0.,effacement=0.):
        if consigne > self.activite: #si on veut augmenter la puissance, on place l'activité actuelle sur la courbe de montée en puissance et on en déduit l'état à t+10min
            ...
            if consigne > self.arret[i+10]:
                return (consigne,0.5*self.varcout*self.activite/100.*self.PROD_MAX*(80./1000./6.)*self.nombre) #voir 8 lignes plus haut pour avoir l'explication du facteur "0.5". c'est assez imprécis comme raisonnement mais pour le moment, il nous manque les données relatives au calcul de ces coûts
            else:
                return (self.arret[i+10],0.5*self.varcout*self.activite/100.*self.PROD_MAX*(80./1000./6.)*self.nombre)
